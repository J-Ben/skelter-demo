...
/* ─── Health ──────────────────────────────────────────────── */
export const HEALTH_FILES: VirtualFile[] = [
  {
    name: 'index.tsx',
    lang: 'tsx',
    code: `'use client';
...
import { fetchHealthProfile } from './api';
import { PLACEHOLDER } from './types';

export default function Health({ delay }: { delay: number }) {
...
function Stat({ icon, value, label }: { icon: string; value: string; label: string }) {
  return (
    <div style={{ flex: 1, background: '#18181b', borderRadius: 10,
      padding: '10px 12px', display: 'flex', flexDirection: 'column' }}>
      <p style={{ fontSize: 11, color: '#71717a', marginBottom: 4, width: 'fit-content' }}>
        {icon} {label}
      </p>
      <p style={{ fontSize: 15, fontWeight: 700, color: '#f4f4f5', width: 'fit-content' }}>
        {value}
      </p>
    </div>
  );
  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        <img src={data.photo} width={72} height={72}
          style={{ borderRadius: '50%', flexShrink: 0, objectFit: 'cover', display: 'block' }} />
        <div>
          <p style={{ fontSize: 16, fontWeight: 700, color: '#f4f4f5',
            width: 'fit-content', marginBottom: 3 }}>
            {data.name}
          </p>
...
export const HealthCard = withSkeleton(HealthCardBase);`,
  },
  {
    name: 'api.ts',
    lang: 'ts',
    code: `import type { HealthProfile } from './types';

export async function fetchHealthProfile(delay: number): Promise<{ data: HealthProfile; loadTime: number }> {
  const t0 = Date.now();
  await new Promise(r => setTimeout(r, delay));

  const res = await fetch(
    'https://randomuser.me/api/?inc=name,picture,dob,location&nat=fr,gb,us,au'
  );
  ...
  },
  {
    name: 'types.ts',
    lang: 'ts',
    code: `export type HealthProfile = {
  name: string;
  age: number;
  photo: string;
  location: string;
...
export const PLACEHOLDER: HealthProfile = {
  name: 'Sophie Martin',
  age: 28,
